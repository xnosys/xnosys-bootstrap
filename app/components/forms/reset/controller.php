...
list($error, $result) = $db['select'](
	...
	array(':id' => $__uuid['unpack'](substr($app['req']['body']['_xr'], 0, 43)))
);
...
# - Checks: to make sure a password reset was requested, and time still valid
if (!strlen($member['getAttr']('password_code_hash'))) { $db['close'](); return array(403); }
if ((time() - strtotime($member['getAttr']('password_code_datetime'))) > 86400) { $db['close'](); return array(403); }

# - verify member attributes
list($error, $success) = $member['verify'](array(
	'password_code_hash' => $member['getAttr']('password_code_salt').substr($app['req']['body']['_xr'], 43)
));
if (!!$error) { $db['close'](); return array(401); }

# - Action: update member password
