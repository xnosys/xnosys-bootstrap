...
# - Checks:
if (isset($app['req']['body']['avatar']) || (isset($app['req']['files']['avatar']) && isset($app['req']['files']['avatar']['tmp_name']))) {
	
	# - Assign: edit avatar
	list($error, $query) = $profile['edit'](array(
		'avatar' => ((isset($app['req']['files']['avatar']) && isset($app['req']['files']['avatar']['tmp_name'])) ? call_user_func(function ($_config, $_upload, $app) {
			list($error, $url) = $_upload(
				$_config,
				$app,
				$app['req']['files']['avatar']['tmp_name'],
				hash('sha512', microtime().rand()).'.'.strtolower(pathinfo($app['req']['files']['avatar']['name'], PATHINFO_EXTENSION))
			);
			if (!!$error) { return ''; }
			return $url;
		}, $_config, $_upload, $app) : (isset($app['req']['body']['avatar']) ? $app['req']['body']['avatar'] : ''))
	));
